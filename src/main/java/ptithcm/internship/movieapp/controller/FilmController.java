...
public class FilmController {
	...
	public String getHotFilm(Model model, @PathVariable("page") int page) {
		...
		int pageCount = (int) Math.ceil(listFilm.size() / (ConstantVariable.MAX_ITEMS * 1d));
		if (page > pageCount) {
			return "redirect:/hotfilm/" + pageCount;
		}
		int currentPage = page;
		List<Integer> pages = new ArrayList<Integer>();
		for (int i = 1; i <= pageCount; i++) {
	...
	public String getCategoryPageByCid(Model model, @PathVariable("cid") String cid, @PathVariable("year") String year) {
		List<Film> listFilm;
		if (cid.equals("ALL")) {
			if (year.equals("ALL")) {
	...
	public String getCategoryPage(Model model, @PathVariable("page") int page, @PathVariable("cid") String cid, @PathVariable("year") String year) {
		if (page < 1) {
	...
	public String getFilmToWatch(Model model,@PathVariable("id") int id) {
		...
		List<Comment> listComment = commentService.findAllByFidWithFirstAndLast(id, PageRequest.of(0,ConstantVariable.DEFAULT_COMMENT));
		if(film.getFdeploy() == 1) {
			model.addAttribute("webDomain", webDomain);
			model.addAttribute("film", film);
	...
	@RequestMapping(value = "/add-new-tv-series", method= RequestMethod.GET)
	public String newFilmTV(Model model) {
		...
		model.addAttribute("listCatagory", listCategory);
		model.addAttribute("listCountry", listCountry);
		return "add_new_film_tv";
	}
	
	@RequestMapping(value = "/add-new-movie", method= RequestMethod.POST)
	public String newFilm(Model model, @ModelAttribute("newFilmRequest") FilmRequest filmRequest, HttpServletRequest request) {
		...
		FilmDetail filmDetail = new FilmDetail(1, filmRequest.getFurl(), currentFilm.getAcceptAccount(), currentFilm.getRequestEmail(), 1, currentFilm);
	...
	@RequestMapping(value = "/add-new-tv-series", method= RequestMethod.POST)
	public String newFilmTV(Model model, @ModelAttribute("newFilmRequest") FilmRequest filmRequest, HttpServletRequest request) {
		...
		for (String furl : filmRequest.getListFilmUrl()) {
			FilmDetail filmDetail = new FilmDetail(episodeCount++, furl, currentFilm.getRequestEmail(), 0, currentFilm);
			filmDetailService.save(filmDetail);
		}
